...
class Library(models.Model):
    ...
    book_ids = fields.Many2many(comodel_name='product.template', relation='library_id', string="Books")
    ...
    _sql_constraints = [
        ('name_unique', 'unique(name)', "This named library is already exist choose different one")
    ]

    @api.constrains('book_ids', 'capacity')
    def _check_capacity(self):
        """ This is the validation that user cannot add books more than library capacity """
        for record in self:
            count = len(record.book_ids)
            if count > record.capacity:
                raise ValidationError(f'Do you want to add more books then increase the capacity of library.'
                                      f'Current capacity is {record.capacity}')
    ...
    @api.depends('book_ids.library_id')
    def _add_library(self):
        for record in self:
            for book in record.book_ids:
                book.library_id = record.name
